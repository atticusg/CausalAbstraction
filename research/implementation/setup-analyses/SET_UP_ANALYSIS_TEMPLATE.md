...
5-section spec consumed by the setup-analyses guide. Lands at `$WORKDIR/code/analyses/<name>/set_up_analysis.md` after the skill writes it. The skill scaffolds `main.py`, `analysis.yaml`, and `README.md` from this spec; downstream, running the experiment references the analysis from a runner config.
...
> Example: *Reads `baseline/per_class_output_dists.safetensors` and `locate/interchange/<variable>/results.json`, then runs an interchange ablation per attention head on the train dataset and records the mean logit-difference shift per (layer, head) cell. Writes one `head_logit_diff/results.json` plus a `(layers × heads)` heatmap figure (PNG by default).*
...
> Example: *None — this is a terminal analysis whose outputs are interpreted downstream by whatever drives the run.*
...
- `$WORKDIR/code/methods/<…>` — session-local primitive you authored (must already be scaffolded via the setup-methods guide).

| Symbol | Source | Notes |
|---|---|---|
| `compute_reference_distributions` | `causalab.methods.metric` | reused as in baseline |
| `head_logit_diff` | `methods.head_logit_diff` (session-local) | scaffolded via the setup-methods guide |

If a session-local method listed here does not yet exist, the skill instructs the user to run the setup-methods guide for it first (passing the missing method's spec path).
...
_subdir: ${.target_variable}_L${.layers[0]}   # pattern on a discriminating field — NOT a fixed literal
```

`_subdir` decisions matter because two runs that share the same `_subdir` overwrite each other. **Never use a fixed literal like `_subdir: default`.** Even when no sweep is *planned*, follow-up or concurrent dispatches (e.g. filling per-layer cells in parallel) write into the same dir and the last writer wins — the `manifold_selfie/default` collision in `patch-scopes-weekdays` (#171 C4). Always pattern `_subdir` on a field that distinguishes the runs you might launch (`${.target_variable}`, `L${.layers[0]}`, …).

If the analysis emits any matplotlib figure, include:

```yaml
visualization:
  figure_format: ${figure_format}   # inherits the global png default; set png/pdf to override — invariant 6
```
...
Every file the analysis writes. Drives both the README "Saved artifacts" table and the run phase's expected-artifact tree.

| File | Format | What it shows | Used by |
|---|---|---|---|
| `results.json` | `{"per_head": {"<L>_<H>": float}, "summary": {...}}` | mean logit-difference shift per cell | interpretation downstream; other analyses |
| `heatmap.png` | matplotlib heatmap | (layers × heads) cell scores | human reference |
| `metadata.json` | run config snapshot | provenance | the run phase's artifact verification |
