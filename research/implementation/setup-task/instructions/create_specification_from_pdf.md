...
Before extracting, read `task_quality_objectives.md` and treat the five objectives there (granularity, grading totality, input determinism, single-token decoding, value coverage) as a checklist. If the paper's task formulation conflicts with any objective (e.g., binary answer for an $N>2$ variable, multi-token answers), surface it to the user before drafting that section.
...
3. Save the draft to: `$WORKDIR/set_up_task_draft.md` (a working directory under your experiment's output)
4. Tell the user: **"Saved draft specification at $WORKDIR/set_up_task_draft.md based on your PDF"**
5. Return control to the main setup-task guide. After final user approval, that guide copies the draft to the task package's `set_up_task.md` (under `$WORKDIR/code/tasks/{task_name}/`) and proceeds to Step 1. Do NOT create the task package from this instruction file — that copy is the outer guide's responsibility.
