...
Run a separate jointly trained mask for every promising combination of layer
band and token location. For spans, test every position separately and the whole
span jointly. Sweep regularization strength when needed to produce a sparsity
curve rather than selecting one arbitrary penalty after seeing the result.

Shard independent band, position, regularization, and seed points within the
attention DBM experiment as described in
...
- selectors for parent band, token position or span, regularization, seed, and
  data split;
