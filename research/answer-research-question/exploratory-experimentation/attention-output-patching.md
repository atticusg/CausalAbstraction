...
```json
"sites":  { "a10": {"component": "attention_output", "layer": 10}, "…": "one per layer" },
...
Copy `causalab/configs/protocols/attention_band_patch.json` and extend it: one
entry per layer in `sites`/`reads`/`writes`, one entry per band in
