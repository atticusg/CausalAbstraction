...
def test_cubic_matches_scipy():
    ...
    # scipy CubicSpline.__call__ returns complex|float NDArray in stubs; runtime is float
    assert abs(ours - ref).max() < 1e-5  # pyright: ignore[reportArgumentType]
...
def test_cubic_smoothing_reduces_roughness():
    ...
    # gamma is a registered buffer (Tensor) but nn.Module typing widens to Tensor|Module
    rough_interp = (interp.gamma**2).sum()  # pyright: ignore[reportOperatorIssue]
    rough_smooth = (smooth.gamma**2).sum()  # pyright: ignore[reportOperatorIssue]
...
def test_periodic_cubic_matches_scipy():
    pytest.importorskip("scipy")
    from scipy.interpolate import CubicSpline as SciCS  # pyright: ignore[reportMissingTypeStubs]
...
def test_periodic_cubic_smoothing_reduces_roughness():
    ...
    assert (smooth.evaluate(cp) - y).abs().max() > 1e-4
    # gamma is a registered buffer (Tensor) but nn.Module typing widens to Tensor|Module
    assert (smooth.gamma**2).sum() < (interp.gamma**2).sum()  # pyright: ignore[reportOperatorIssue]
