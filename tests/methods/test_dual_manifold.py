...
class TestFromEvaluateArtifacts:
    ...
    def test_basic_shapes(self):
        from causalab.analyses.path_steering.dual_manifold import DualManifoldData
        ...
        # artifacts is dict[str, Union[...]]; element types are correct at runtime
        data = DualManifoldData.from_evaluate_artifacts(**artifacts)  # pyright: ignore[reportArgumentType]
    ...
    def test_belief_paths_embed_then_average_over_prompts(self):
        """Belief path = embed each prompt's per-step dist, then average in 3D.

        Regression guard for #209: the old code embedded the prompt-mean
        probability (``PCA(√ mean_p p)``); averaging before the concave ``√``
        collapses peaky dists toward the simplex barycenter. The fix averages the
        *embedded* per-prompt points instead (``mean_p PCA(√ p)``).
        """
        from causalab.analyses.path_steering.dual_manifold import DualManifoldData
        ...
        # geo_distributions value is Tensor at runtime; dict's Union type can't be narrowed
        geo0 = artifacts[
            "geo_distributions"
        ][
            0
        ].numpy()  # (n_steps, n_prompts, W)  # pyright: ignore[reportIndexIssue, reportOptionalSubscript, reportAttributeAccessIssue]
        n_prompts = geo0.shape[1]

        # embed-then-average: Hellinger-embed each prompt, then mean over prompts
        embedded = []
        for p in range(n_prompts):
            d = geo0[:, p, :]
            other = np.clip(1.0 - d.sum(axis=-1, keepdims=True), 0, None)
            full = np.concatenate([d, other], axis=-1)
            # hellinger_pca value is sklearn PCA at runtime; dict's Union can't be narrowed
            embedded.append(
                hellinger_pca.transform(  # pyright: ignore[reportCallIssue, reportAttributeAccessIssue, reportOptionalMemberAccess]
                    np.sqrt(np.clip(full, 0, None)).astype(np.float32)
                )
            )
        expected_3d = np.stack(embedded, axis=0).mean(axis=0)
        np.testing.assert_allclose(data.geo_bel_paths_3d[0], expected_3d, atol=1e-5)

        # ...and must differ from the old average-then-embed embedding.
        mean_dists = geo0.mean(axis=1)
        other = np.clip(1.0 - mean_dists.sum(axis=-1, keepdims=True), 0, None)
        old_3d = hellinger_pca.transform(  # pyright: ignore[reportCallIssue, reportAttributeAccessIssue, reportOptionalMemberAccess]
            np.sqrt(
                np.clip(np.concatenate([mean_dists, other], axis=-1), 0, None)
            ).astype(np.float32)
        )
        assert np.abs(data.geo_bel_paths_3d[0] - old_3d).max() > 1e-4

    def test_belief_centroids_match_scatter_cloud_means(self):
        """Belief centroids = per-class mean of the embedded background scatter.

        Regression guard for #209 (Evidence 2): centroids must land on their
        scatter clouds at data scale, not collapse to a central knot built from
        a different (steering-path-endpoint) population.
        """
        from causalab.analyses.path_steering.dual_manifold import DualManifoldData
        ...
        n_classes, W = 5, 5
        n_per = 12
        classes = np.repeat(np.arange(n_classes), n_per)
        # Peaky per-class natural dists so each class's scatter clusters distinctly.
        dists = np.full((len(classes), W + 1), 0.02, dtype=np.float64)
        for i, c in enumerate(classes):
            dists[i, c] = 1.0
        dists += rng.dirichlet(np.ones(W + 1), size=len(classes)) * 0.1
        dists = dists / dists.sum(axis=1, keepdims=True)

        artifacts = _make_test_artifacts(n_classes=n_classes, n_pairs=2, n_steps=8, W=W)
        artifacts["natural_dists"] = torch.tensor(dists, dtype=torch.float32)
        artifacts["bel_class_assignments_true"] = classes
        # artifacts is dict[str, Union[...]]; element types are correct at runtime
        data = DualManifoldData.from_evaluate_artifacts(**artifacts)  # pyright: ignore[reportArgumentType]

        # Each centroid is exactly the mean of its class's embedded scatter points.
        for c in range(n_classes):
            mask = data.bel_class_assignments == c
            expected = data.bel_background_3d[mask].mean(axis=0)
            np.testing.assert_allclose(data.bel_centroids_3d[c], expected, atol=1e-5)

        # Centroids are spread across the panel, not collapsed to one point.
        assert data.bel_centroids_3d.std(axis=0).max() > 1e-2
