...
``restore.component_type`` string to the right spec-grid builder in
``_build_grid_sites`` (the restore grid). The builder layer is unit-tested in
``tests/neural/activations/test_site_grids.py``; this module pins the
*dispatch* — that each component string reaches its builder and produces specs
keyed the way the scan / save path expects — plus the registered corruption
kinds and component types.
"""
...
class TestWindowing:
    ...
    def test_window_keys_by_center_and_collects_window_units(
        ...
        assert set(targets.keys()) == {(0, "last_token"), (1, "last_token")}
        # A window=2 cell at center 0 restores 2 layers' worth of specs
        # ([0, 1]); the count matches _window_layers under clamping. Windowed
        # cells are single-group nestings (one joint restore group).
        assert len(targets[(0, "last_token")]) == 1
        assert len(targets[(0, "last_token")][0]) == len(_window_layers(0, 2, n_layers))
        assert len(targets[(0, "last_token")][0]) == 2

    def test_window_1_is_single_spec_per_cell(self, mock_tiny_lm: LMPipeline) -> None:
        span = _last_token(mock_tiny_lm)
        n_layers = mock_tiny_lm.model.config.num_hidden_layers
        targets = _windowed_swept_targets(
