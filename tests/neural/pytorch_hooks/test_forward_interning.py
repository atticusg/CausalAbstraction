"""Cross-point forward interning in the reference backend (spec §3, §4).
...
itself, not the backend's own tally, so a bookkeeping bug cannot make the
...
from causalab.neural.pytorch_hooks import PytorchHooksBackend
from causalab.neural.pytorch_hooks.backend import campaign_plans
...
def forwards() -> Iterator[list[int]]:
    ...
    ``load_model`` is memoized on its exact call form, so asking for it the
    way the backend does hands back the very object the backend will run — no
    patching, and nothing private touched."""
    bundle = load_model(TINY_LLAMA, "main", dtype="fp32", device="cpu")
    calls: list[int] = []
...
def test_a_shared_forward_group_runs_once(
    ...
    """The gap this closes: the backend runs the campaign's distinct forward
    ...
    result = PytorchHooksBackend().execute(_request(scan, scan_env, tmp_path))
    ...
    # the backend's own tally must agree with what the model actually saw
...
def test_interning_changes_no_number(
    ...
    """
    whole = PytorchHooksBackend().execute(_request(scan, scan_env, tmp_path / "whole"))
    shards = [
        PytorchHooksBackend().execute(
...
def test_a_lone_point_still_runs_its_own_groups(
    ...
    result = PytorchHooksBackend().execute(_request(scan, scan_env, tmp_path, [0]))
