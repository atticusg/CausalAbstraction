...
def diag_featurizer(d: int) -> Featurizer:
    """A non-identity :class:`Featurizer` pair sized for a single position."""
    fwd = DiagFeaturizerModule(d)
    return Featurizer(
        featurizer=fwd,
        inverse_featurizer=DiagInverseFeaturizerModule(fwd.weight),
    )
...
def rotate_featurizer(R: torch.Tensor) -> Featurizer:
    """A non-identity :class:`Featurizer` rotating into orthonormal basis ``R``."""
    return Featurizer(
        featurizer=RotateFeaturizerModule(R),
        inverse_featurizer=RotateInverseFeaturizerModule(R),
