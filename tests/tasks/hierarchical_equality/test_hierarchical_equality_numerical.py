"""Numerical-tier pin for the hierarchical_equality task's symbolic outputs.
...
counterfactual generator). It is **not** a "golden" — goldens are the
runner-scope, full-pipeline pins under ``tests/end_to_end/goldens/``
that exercise a small model which reliably solves the task. See
docs/TESTS.md "Task numerical pins" vs "Runner-golden standard" for the
distinction.

Refresh the sidecar via::

    uv run python scripts/update_task_pins.py --task=hierarchical_equality
    uv run python scripts/update_task_pins.py --task=hierarchical_equality \\
...
from causalab.tasks.hierarchical_equality import causal_models, counterfactuals  # noqa: F401
...
def test_pinned_samples() -> None:
    ...
    actual = walk_task_samples("hierarchical_equality")
    ...
    assert len(actual_samples) == len(expected_samples), (
        ...
        f"`uv run python scripts/update_task_pins.py --task=hierarchical_equality "
