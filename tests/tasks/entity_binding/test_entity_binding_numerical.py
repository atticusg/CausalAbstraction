"""Numerical-tier pin for the entity_binding task's symbolic outputs.
...
Refresh the sidecar via::
    ...
    uv run python scripts/update_task_pins.py --task=entity_binding
    uv run python scripts/update_task_pins.py --task=entity_binding \\
...
from causalab.tasks.entity_binding import causal_models, counterfactuals  # noqa: F401
...
def test_pinned_samples() -> None:
    ...
    actual = walk_task_samples("entity_binding")
    ...
    assert len(actual_samples) == len(expected_samples), (
        ...
        f"`uv run python scripts/update_task_pins.py --task=entity_binding "
