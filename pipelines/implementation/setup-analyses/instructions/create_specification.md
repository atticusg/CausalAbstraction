...
This file is read by the setup-analyses guide's Step 1 when no spec paths are provided (single-spec interactive elicitation fallback). Walk the user through the 5 sections of `SET_UP_ANALYSIS_TEMPLATE.md` one at a time, getting approval after each section.

Tell the user up front:

> "I'll guide you through creating an analysis spec. We'll fill out 5 sections (identity, position in DAG, methods used, config schema, outputs). I'll save the draft to `$WORKDIR/code/analyses/<name>/set_up_analysis.md` as we go and ask for approval after each section."

While iterating, write the partial draft to `$WORKDIR/code/analyses/<name>/set_up_analysis.md` so the user can see the cumulative result.
...
Validate: must be `[a-z][a-z0-9_]*`, must not collide with a shipped `causalab/analyses/<name>/` or `causalab/configs/analysis/<name>.yaml`, nor an existing `$WORKDIR/code/analyses/<name>/` (a session-local name must not shadow a shipped analysis). If it collides, ask again.
...
- **`methods.<name>` (session-local)** — anything you scaffolded via the setup-methods guide.

For each session-local method listed:
1. Confirm the file `$WORKDIR/code/methods/<name>/<name>.py` exists.
2. If it doesn't, advise the user to run the setup-methods guide for `<name>` *before* finishing this spec.
...
If the analysis emits a heatmap figure, capture the row/column semantics so the README can document them.
...
2. Write to `$WORKDIR/code/analyses/<name>/set_up_analysis.md` (creating the directory if needed).
3. Tell the user: **"Spec saved at `$WORKDIR/code/analyses/<name>/set_up_analysis.md`. Proceeding to scaffold."**
