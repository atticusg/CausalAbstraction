...
This file is read by the setup-methods guide's Step 1 when no spec paths are provided (single-spec interactive elicitation fallback). Walk the user through the 5 sections of `SET_UP_METHOD_TEMPLATE.md` one at a time, getting approval after each section.

Tell the user up front:

> "I'll guide you through creating a method spec. We'll fill out 5 sections (identity, surface, dependencies, hyperparameters, side effects). I'll save the draft to `$WORKDIR/code/methods/<name>/set_up_method.md` as we go and ask for approval after each section."

While iterating, write the partial draft to `$WORKDIR/code/methods/<name>/set_up_method.md` so the user can see the cumulative result. Write nothing to `$WORKDIR/code/methods/<name>/<name>.py` until Step 3 of the main workflow.
...
Validate: must be `[a-z][a-z0-9_]*`, must not collide with a shipped `causalab/methods/<name>/` or `causalab/methods/<name>.py`, nor an existing `$WORKDIR/code/methods/<name>/`. If it collides, ask again.
...
Make sure every input has a documented shape (or a clear "scalar"/"sequence of strings") and every output is named. The signature lines up with what the setup-analyses guide will see when it lists §3 dependencies.
...
If the user insists on disk I/O at the method layer, surface the layering exception to the user so it stays visible.
...
2. Write to `$WORKDIR/code/methods/<name>/set_up_method.md` (creating the directory if needed).
3. Tell the user: **"Spec saved at `$WORKDIR/code/methods/<name>/set_up_method.md`. Proceeding to scaffold."**
