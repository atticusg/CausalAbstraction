...
5-section spec consumed by the setup-methods guide. Lands at `$WORKDIR/code/methods/<name>/set_up_method.md` after the skill writes it. The skill scaffolds code from this spec; downstream the setup-analyses guide reads it to know what symbol to import.
...
**Rule:** every keyword argument that is a *hyperparameter* (subspace dim, learning rate, regularization coefficient, batch size, …) must have **no default value**. Defaults live in Hydra (`docs/CODEBASE.md` §3 invariant 5).
