...
def create_token_positions(
    ...
    Returns:
        Dictionary mapping position names to TokenPosition instances
    """
    ...
    # IMPORTANT: Document why you needed the fallback!
    ...
    # build_token_positions() builds the declarative specs and materializes them into
    # TokenPosition instances bound to `pipeline` — the shape consumers expect (they
    # read `.id` off each value). Return its result directly; do NOT return the raw
    # factories from build_token_position_factories(), which crashes the `locate` step
    # with `AttributeError: 'function' object has no attribute 'id'`.
