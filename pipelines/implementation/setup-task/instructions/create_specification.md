...
The specification is a markdown file drafted at `$WORKDIR/set_up_task_draft.md` (a working directory under your experiment's output) and copied to the task package's `set_up_task.md` (under `$WORKDIR/code/tasks/{task_name}/`) after the user approves it (the outer setup-task guide handles that copy — see `../setup-task.md` Step 1/Step 2). It captures every decision needed to implement the task. The structure is defined inline below (Sections 1–8 + Output Token Mode); there is no separate TEMPLATE.md file to read from.

For reference on how implemented tasks look, browse `causalab/tasks/{name}/` (e.g. `MCQA`, `hierarchical_equality`, `weekdays`) — each contains the implementation files that this specification will produce.

Before drafting, read `task_quality_objectives.md` and treat the five objectives there (granularity, grading totality, input determinism, single-token decoding, value coverage) as a checklist. Surface concerns to the user as you elicit each section.
...
3. Save the draft to: `$WORKDIR/set_up_task_draft.md`
4. Tell the user: **"Saved draft specification at $WORKDIR/set_up_task_draft.md"**
5. Return control to the main setup-task guide. After final user approval, that guide copies the draft to the task package's `set_up_task.md` (under `$WORKDIR/code/tasks/{task_name}/`) and proceeds to Step 2. Do NOT create the task package from this instruction file — that copy is the outer guide's responsibility.
