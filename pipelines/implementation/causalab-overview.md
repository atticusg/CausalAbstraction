...
The single source of truth for causalab *the codebase*: the workflow layout, the key directories, how an experiment runs, and the analyses and turnkey pipelines on offer. The method-agnostic *science* lives in the causal-analysis handbook (`plugins/capabilities/research-handbooks/causal-handbook.md`); this doc documents how that science runs on causalab. It is a concise companion to the in-repo docs — `~/.silico/libraries/causalab-internal/docs/CODEBASE.md` is authoritative for architecture, layering, and invariants, and each analysis ships its own `README.md`; consult those when this overview and the code disagree.

## The workflow layout

In silico the causalab science is exposed as the documents of the `causalab-pipeline` skill, backed by the method-agnostic handbook the planner works from:

- **the causal-analysis handbook** (`plugins/capabilities/research-handbooks/causal-handbook.md`) — the method-agnostic science the planner reasons from.
- **The research workflows** — `../answer-research-question/hypothesis-generation/hypothesis-generation.md` (task → causal models + counterfactuals), `../answer-research-question/exploratory-experimentation/exploratory-experimentation.md` (a behavior/model → initial task + preliminary experiments), `../explore-subspace/explore-subspace.md` (a subspace → autointerp), and the two end-to-end autonomous pipelines `../subspace-causal-analysis-pipeline/subspace-causal-analysis-pipeline.md` (is this given subspace causal?) and `../answer-research-question/answer-research-question.md` (which subspace mediates this task?).
...
- Use the handbook's methods (correlation vs. causation, designing causal models & counterfactual datasets, choosing an intervention, quantify & report), adapted to the model by hand.
- The develop-causal-model step is **CPU-only and fully model-agnostic** — a high-level causal model plus the counterfactual datasets that distinguish hypotheses — so `../answer-research-question/hypothesis-generation/hypothesis-generation.md` applies directly regardless of the target model.
- Write results up against the report-format templates in [`references/`](references/) and in `../answer-research-question/hypothesis-generation/hypothesis-generation.md` — they hold no matter which model or tool produced the numbers.
...
When you do choose to run on causalab, two turnkey pipelines wrap the whole chain so you needn't wire the stages by hand — pick by what you start from:
...
- **`../answer-research-question/answer-research-question.md`** — start from a **task/behavior** ("which subspace mediates this latent?"). Drives develop-causal-model → setup-task → plan → run → interpret end to end, discovering the mediating subspace via learned DAS.
...
- the develop-causal-model report-format reference (in `../answer-research-question/hypothesis-generation/hypothesis-generation.md`) — the fixed tone, macrostructure, per-dataset distinguishability-heatmap rules, and figure-caption convention for a develop-causal-model report (competing causal models + counterfactual datasets and their CPU distinguishability).
