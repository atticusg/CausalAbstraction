...
|---|---|---|---|
| `baseline` | Can the model solve the task? | — | `accuracy.json`, `per_class_output_dists.safetensors` |
...
| `path_steering` | Does the subspace/manifold faithfully preserve causal structure? (isometry, coherence, conformal) | `subspace`, `activation_manifold` | per-metric scores and plots, `dual_manifold` + `receptive_field` viewers, `path_visualization/` |
...
## Designing sign-of-life sweeps

A pre-flight gate is only useful if it *could plausibly fail*. Each gate names two cases, and the gap between them is the search space the rest of the sweep narrows.

1. **Informative gate.** A (knob, value) whose outcome would actually update belief about whether this analysis works *on this task and model*. Pair a should-pass case (expected to work) with a should-fail case (expected to fail), each with a reason.
2. **Non-informative gate — does not count.** An outcome dictated by construction: a dead token, BOS with no preceding context, an empty subspace, a degenerate metric. *Example of a rigged should-fail:* `locate` at `L=0, position=BOS → expect IIA ≈ 0`. No model has the counterfactual signal there, so the gate tells us nothing. Reject these.
3. **Find extremes, then narrow.** Run the should-pass and should-fail cases first (cheap, small dataset). If a clear signal separates them in both directions, narrow the knob range and run the full sweep. If not, the gate or the setup is wrong — fix one knob and retry (see `running-experiments.md` § "The gate concept"; whatever drives the run enforces this iterate-until-signal loop between stages).

### Worked examples

| Analysis | Should-pass (expected to work) | Should-fail (informative) | What the gap measures |
|---|---|---|---|
| `locate` | a middle-layer cell that should carry the variable → ≥ 1 cell with elevated `KL_drop` / interchange accuracy | an early layer **before** the variable is written into the residual stream → no elevated cell | the layer band where the variable becomes causally available |
| `subspace` | `k = d_model // 4` → reconstruction near the top (low `reconstruction_kl` / high `causal_alignment`) | `k = d_model // 32` → reconstruction near the bottom | whether the variable lives in a low-dimensional subspace, and how low `k` can go |
| `path_steering` | a pair of geodesic endpoints with **known small** semantic distance → high isometry / coherence | a pair drawn **across different variables** (semantically unrelated) → low isometry | whether the manifold preserves *this variable's* causal structure rather than arbitrary geometry |

For a gap (custom) analysis, draft the pair against its intended output schema (the analysis spec's §5) using the same shape: one knob value expected to work, one expected to fail informatively.

---

## Runner YAML Patterns

Runner configs are primary Hydra configs selected with `--config-name <name>`. Shipped presets live under `causalab/configs/runners/<group>/` in the library checkout and are read-only starting points to copy from; runners you author live **out of tree** under `$WORKDIR/code/configs/runners/<group>/` and are picked up when the run sets `CAUSALAB_SESSION_CODE=$WORKDIR`. Because a runner sits in a sub-directory of the config root, it must start with `# @package _global_` and use **absolute** (leading-`/`) defaults paths (`- /base`, `- /task: …`, `- /analysis/<name>`) — a bare `- analysis/<name>` resolves relative to the runner's own location and fails to load (`Could not load 'runners/.../analysis/<name>'`). Each inherits `base` for shared globals, declares task and model directly (no `override`), and lists analysis steps in the `defaults:` block. See `docs/CODEBASE.md` §2.
...
The wrapper script `scripts/run_exp.sh` is the single entry point for both inline and slurm runs. It passes `--config-name <name>` to Hydra and dispatches to `causalab.runner.run_exp`. Run it from the causalab library checkout — every command block below assumes you have `cd ~/.silico/libraries/causalab-internal` first. To use a session-local runner/task/analysis authored under `$WORKDIR/code/`, also `export CAUSALAB_SESSION_CODE="$WORKDIR"` first (the wrapper then adds `$WORKDIR/code` to PYTHONPATH + Hydra's search path); the shipped-preset examples below don't need it.
...
Per docs/CODEBASE.md §3 invariant 7, all outputs go to `${experiment_root}/{analysis}/...`. Left unset, `experiment_root` defaults to `artifacts/{task}/{model}`; pass `--experiment-root $WORKDIR/artifacts/{task}/{model}` (a working directory under your experiment's output) so outputs land where you collect them. Re-running the same runner config rewrites that directory — there is no automatic timestamping. To preserve an old run, copy the artifact directory aside or branch the runner config and rename it before re-running.
...
`locate.mode: pairwise` is only informative when `task.resample_variable` is set to a single variable name (the variable being localized). Using `pairwise` with `resample_variable: "all"` produces near-zero scores everywhere except the last token. See **docs/CODEBASE.md §5** for the full explanation. Centroid mode (`locate.mode: centroid`) remains meaningful under `resample_variable: "all"` — use it for variable-agnostic localization.
...
- **`mode`**: `pairwise` (default — scores each cell per example against the causal model's expected counterfactual label (`task.intervention_metric: causal_label`/`string_match`) or the base output distribution (`output_shift`); only informative when `task.resample_variable` is the specific variable being localized — see docs/CODEBASE.md §5) vs `centroid` (distributions compared class-by-class; works with any counterfactual type, incl. `resample_variable: "all"`).
...
### `subspace`
- **`method`**: `pca` (no training, fast); `das` (supervised linear subspace, learns rotation that maximizes causal alignment); `dbm` (binary mask over feature dimensions); `boundless` (Boundless DAS); `fixed` (thread a **given/precomputed** rotation — SAE decoder directions, imported basis — through the pipeline without fitting).
- **`k_features`**: subspace dimensionality. Rule of thumb: 2–3× the number of distinct variable values for PCA; can be lower for DAS/DBM. Start with 8–16 and compare. For `method: fixed`, set it to the given rotation's `k`.
- **Threading a given subspace?** Use `method: fixed` with `subspace.fixed.{artifact|source|feature_ids}` and a single explicit `layers: [L]`. The artifact is produced by `characterize_subspace/subspace_builder.py` (SAE cluster → `.safetensors`). **Do NOT scaffold a session-local `fixed_subspace` analysis** — the shipped `subspace method: fixed` already writes the bundle `activation_manifold`/`path_steering` auto-discover (including `raw_features` for the linear path mode).
...
- **`visualizations`**: which viz to render (shipped default `[path_visualization, isometry_visualization, dual_manifold]`). Append `receptive_field` to also render the **receptive-field decision map** — a 2-D/3-D grid over the top subspace PCs where each cell is steered and colored by its argmax output class, with interactive slice / class / path controls and centroid + point-cloud + steering-path overlays. It is **opt-in** (extra `grid_res^D × n_prompts` forward passes, so it doesn't burden the golden tier) but is **standard for the artifact viewer**: enable it whenever the §F viewer selection includes the receptive-field figure (the default when `path_steering` is in the DAG). Per-viz knobs (`grid_res`, `pca_components` — 2 dims → 2-D, 3 → 3-D, `n_prompts`, …) live under a `receptive_field:` block; see `causalab/configs/analysis/path_steering.yaml`. Output: `path_steering/**/vis/receptive_field.html`.
