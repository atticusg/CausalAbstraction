...
class ResolvedSite:
    ...
    :func:`~causalab.protocol.registry.component_shape`, so the description the
    backend converts by and the description the protocol layer validates against
    are the same object. ``tuple_index`` defaults to the historical rule —
    element 0 of a tuple payload.
    """
...
def _embedding(bundle: ModelBundle) -> Any:
    return (
...
def _head_slice(bundle: ModelBundle, component: str, head: int | None) -> slice | None:
...
def _q_projection_splits(bundle: ModelBundle, attn: Any, layer: int) -> int:
...
_FULL_ATTENTION_ONLY: frozenset[str] = frozenset(
    {"attention_probs"} | _ATTENTION_INTERIOR | set(ATTENTION_FUNCTION_SLOTS)
)
