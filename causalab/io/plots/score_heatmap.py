...
def plot_score_heatmap(
    ...
    interchange_targets: SiteGrid,
    title: str,
    save_path: str,
    figure_format: str = "png",
) -> None:
    """
    Plot score heatmap, auto-detecting component type from the spec grid.

    This unified dispatcher structurally detects whether the grid targets
    attention heads, residual stream positions, or MLPs, and calls the
    ...
    Args:
        ...
        interchange_targets: A built spec grid
            (:mod:`causalab.neural.activations.site_grids`). Used to detect
            component type and extract grid dimensions.
        title: Plot title.
        save_path: Path to save the figure.
        figure_format: ``png`` or ``pdf`` for static output.
    """
    # Detect component type (structural — reads the first spec's engine site)
    component_type = grid_component(interchange_targets)
