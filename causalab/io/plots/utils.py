...
def show_current_figure() -> None:
    """Show the active figure without a GUI backend (e.g. Agg in VS Code notebooks).

    Renders to PNG bytes so the image displays reliably; ``display(fig)`` alone can
    print only ``<Figure ...>`` text in some frontends, especially if the figure is
...
def create_heatmap(
    ...
    **kwargs: Any,
    ...
    Args:
        score_matrix: 2D numpy array of scores (typically in range 0.0-1.0)
        ...
        figure_format: ``png`` or ``pdf`` (replaces ``save_path`` extension when saving).
