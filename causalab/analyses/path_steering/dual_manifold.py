...
from causalab.io.plots.plot_3d_interactive import (
    ...
    _build_manifold_mesh_traces,  # pyright: ignore[reportPrivateUsage]
    _make_edge_traces,  # pyright: ignore[reportPrivateUsage]
)
from .path_visualization import (
    _is_2d_spatial,  # pyright: ignore[reportPrivateUsage]
    _build_grid_layout,  # pyright: ignore[reportPrivateUsage]
    _build_rc_to_w,  # pyright: ignore[reportPrivateUsage]
    _get_grid_row_colors,  # pyright: ignore[reportPrivateUsage]
)
...
def _class_mean_centroids(
    background_3d: np.ndarray,  # (n_examples, 3) embedded scatter
    class_assignments: np.ndarray,  # (n_examples,) class index per row
    n_classes: int,
) -> np.ndarray:
    """Belief centroids as the per-class mean of the embedded background scatter.

    The Hellinger embedding ``φ(p) = PCA(√p)`` does not commute with averaging in
    probability space (``√`` is concave), so the correct class centroid is the
    mean of the *embedded* per-example points, ``meanᵢ φ(pᵢ)`` — not
    ``φ(meanᵢ pᵢ)``. Classes with no background examples are placed at the origin
    (the PCA-centered grand mean) and warned about. See issue #209.
    """
    centroids = np.zeros((n_classes, 3), dtype=np.float32)
    for ci in range(n_classes):
        mask = class_assignments == ci
        if mask.any():
            centroids[ci] = background_3d[mask].mean(axis=0)
        else:
            logger.warning(
                "Belief centroid for class %d has no background examples; "
                "placing at origin.",
                ci,
            )
    return centroids
...
class DualManifoldData:
    ...
    def from_evaluate_artifacts(
        ...
        bel_class_assignments_true: np.ndarray
        | None = None,  # TRUE labels for natural_dists rows (required)
    ) -> "DualManifoldData":
        ...
        def _embed_then_average_over_prompts(dists_3d: Tensor) -> np.ndarray:
            """Belief path embedding, prompt-averaged in the embedded 3D space.

            ``dists_3d`` is ``(n_steps, n_prompts, W)``. We Hellinger-embed each
            prompt's per-step distribution and then average the resulting 3D
            points over prompts — embed-then-average. Averaging the probability
            vectors first (``√`` of the prompt-mean) collapses peaky dists toward
            uniform, whose ``√``-image is a tight central blob (the #209 bug),
            because ``√`` is concave: ``√(mean p) ≠ mean √(p)``.
            """
            arr = dists_3d.detach().cpu().float().numpy()
            n_prompts = arr.shape[1]
            embedded = np.stack(
                [_to_hellinger_3d(arr[:, p, :]) for p in range(n_prompts)],
                axis=0,
            )  # (n_prompts, n_steps, 3)
            return embedded.mean(axis=0)  # (n_steps, 3)
        ...
        # geo_mean (prompt-averaged) feeds only the per-step dist line/bar charts,
        # which read raw probabilities and are correct as-is.
        geo_mean = geo_distributions.mean(dim=2)  # (n_pairs, n_steps, W)
        geo_bel_paths_3d = [
            _embed_then_average_over_prompts(geo_distributions[i])
        ...
        if (
            ...
            lin_bel_paths_3d = [
                _embed_then_average_over_prompts(lin_distributions[i])
                for i in range(n_pairs)
            ]
        ...
        # Belief centroids = per-class mean of the embedded background scatter
        # (embed-then-average). Embedding the prompt-/class-averaged probability
        # vector — PCA(√ mean p) of steering-path endpoints, a different
        # population — collapsed every centroid to a central knot off the clouds
        # (the #209 bug); averaging in the embedded 3D space lands each centroid
        # on its cloud at data scale.
        bel_centroids_3d = _class_mean_centroids(
            bel_background_3d, bel_class_assignments, n_classes
        )
    ...
    def from_pullback_artifacts(
        ...
        # Belief centroids = per-class mean of the embedded background scatter
        # (embed-then-average); the old PCA(√ mean endpoints) collapsed them to a
        # central knot — the #209 bug, which the pullback path shared.
        bel_centroids_3d = _class_mean_centroids(
            bel_background_3d, bel_class_assignments, n_classes
        )
...
def _decode_manifold_curve(
    ...
    if d == 1:
        ...
        if periodic and manifold.periods:
            ...
            lo = cp[:, 0].min().item()  # pyright: ignore[reportIndexIssue]  # SplineManifold.control_points is a buffer Tensor; Module __getattr__ widens type
            hi = cp[:, 0].max().item()  # pyright: ignore[reportIndexIssue]  # SplineManifold.control_points is a buffer Tensor; Module __getattr__ widens type
    ...
    t = t.to(device)  # pyright: ignore[reportCallIssue, reportArgumentType]  # Module __getattr__ widens buffer Tensor type
    mean = mean.to(device)  # pyright: ignore[reportCallIssue, reportArgumentType]  # Module __getattr__ widens buffer Tensor type
    std = std.to(device)  # pyright: ignore[reportCallIssue, reportArgumentType]  # Module __getattr__ widens buffer Tensor type
    with torch.no_grad():
...
def _add_scatter_and_manifold(
    ...
    first_centroid_trace_idx.append(len(fig.data))  # pyright: ignore[reportArgumentType]  # plotly Figure.data is a tuple-like Sized container; stubs widen to Figure
...
def _build_figure(
    ...
    has_linear: bool,
    edges: list | None = None,
) -> tuple[go.Figure, int, int, int, int]:
    """Build a styled 3D figure.

    Returns (fig, geo_line_idx, lin_line_idx, geo_point_trace_idx, lin_point_trace_idx).
    lin_line_idx and lin_point_trace_idx == -1 if no linear paths.
    """
    fig = go.Figure()
    ...
    z_floor = fg._get_z_floor(fig)  # pyright: ignore[reportPrivateUsage]
    ...
    geo_line_idx = len(fig.data) - 1  # pyright: ignore[reportArgumentType]  # plotly Figure.data is a tuple-like Sized container; stubs widen to Figure
    ...
    if has_linear and lin_first is not None:
        ...
        lin_line_idx = len(fig.data) - 1  # pyright: ignore[reportArgumentType]  # plotly Figure.data is a tuple-like Sized container; stubs widen to Figure
    ...
    geo_pt_idx = len(fig.data) - 1  # pyright: ignore[reportArgumentType]  # plotly Figure.data is a tuple-like Sized container; stubs widen to Figure
    ...
    if has_linear and lin_first is not None:
        ...
        lin_pt_idx = len(fig.data) - 1  # pyright: ignore[reportArgumentType]  # plotly Figure.data is a tuple-like Sized container; stubs widen to Figure
...
def _build_grid_flow_html(
    ...
    rc_to_w = _build_rc_to_w(grid_values, class_to_rc)
...
def save_dual_manifold_html(
    ...
    if is_grid_mode and data.grid_values and _is_2d_spatial(data.grid_values):
        n_rows, n_cols, _, _, class_to_rc = _build_grid_layout(data.grid_values)
    ...
    lin_first_act = (
        data.lin_act_paths_3d[0] if data.lin_act_paths_3d is not None else None
    )
    geo_first_bel = (
        data.geo_bel_paths_3d[0] if data.geo_bel_paths_3d else np.zeros((1, 3))
    )
    lin_first_bel = (
        data.lin_bel_paths_3d[0] if data.lin_bel_paths_3d is not None else None
    )
    ...
    act_html = pio.to_html(
        act_fig,
        full_html=False,
        include_plotlyjs="cdn",  # pyright: ignore[reportArgumentType]  # plotly accepts str literals like "cdn" at runtime; stubs only declare bool
        div_id="act-plot",
    )
    ...
    lin_act_json = (
        ...
        if data.lin_act_paths_3d is not None
        else "null"
    )
    lin_bel_json = (
        json.dumps([p.tolist() for p in data.lin_bel_paths_3d])
        if data.lin_bel_paths_3d is not None
        else "null"
    )
    lin_dists_json = (
