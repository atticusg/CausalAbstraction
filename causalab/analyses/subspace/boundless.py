"""Boundless DAS (per-feature mask) subspace discovery — single-cell mode.

Trains binary masks over feature dimensions (not units) to identify which
feature dimensions within each unit encode a causal variable.

Key difference from DBM (single-cell):
- DBM (tie_masks=True): one mask per unit — unit selected/deselected entirely.
- Boundless DAS (tie_masks=False): one mask per feature dimension — fine-grained
  selection within each unit.
"""

from __future__ import annotations

import logging
import os
from typing import Any, Callable, cast

from causalab.causal.causal_model import CausalModel
from causalab.neural.units import InterchangeTarget
from causalab.neural.pipeline import LMPipeline

logger = logging.getLogger(__name__)


def find_boundless_subspace(
    target: InterchangeTarget,
    train_dataset: list,
    test_dataset: list,
    pipeline: LMPipeline,
    causal_model: CausalModel,
    k_features: int,
    batch_size: int = 32,
    output_dir: str = "",
    metric: Callable | None = None,
) -> dict[str, Any]:
    """Train Boundless DAS (per-feature masks) to identify important feature dimensions.

    Delegates to the modern ``train_interventions`` path with
    ``intervention_type="mask"`` and ``tie_masks=False``, matching the behavior
    of :func:`~causalab.analyses.subspace.grid.run_boundless_grid` for the
    single-cell case.

    Args:
        target: Interchange target.
        train_dataset: Training counterfactual examples.
        test_dataset: Test counterfactual examples.
        pipeline: LM pipeline.
        causal_model: Causal model for intervention training.
        k_features: Number of features in the DAS subspace.
        batch_size: Training/evaluation batch size.
        output_dir: Where to save Boundless DAS artifacts.
        metric: Intervention success metric.

    Returns:
        Dict with key ``boundless_result`` containing:
        - ``train_score``: Training accuracy.
        - ``test_score``: Test accuracy.
        - ``n_features_by_unit``: Number of selected feature dimensions per unit.
    """
    from causalab.methods.trained_subspace.train import train_interventions
    from causalab.configs.train_config import merge_with_defaults

    os.makedirs(output_dir or ".", exist_ok=True)

    mask_config = merge_with_defaults(
        {
            "intervention_type": "mask",
            "DAS": {"n_features": k_features},
            "featurizer_kwargs": {
                "tie_masks": False
            },  # Boundless DAS: per-dimension masks
            "train_batch_size": batch_size,
            "evaluation_batch_size": batch_size,
        }
    )
    mask_config["log_dir"] = (
        os.path.join(output_dir or ".", "logs") if output_dir else "logs"
    )

    if metric is None:
        raise ValueError("`metric` is required for find_boundless_subspace")
    result = train_interventions(
        causal_model=causal_model,
        interchange_targets={("single",): target},
        train_dataset=train_dataset,
        test_dataset=test_dataset,
        pipeline=pipeline,
        target_variable_group=("raw_output",),
        metric=metric,
        # ExperimentConfig (TypedDict) is structurally compatible with dict[str, Any]
        config=cast(dict[str, Any], mask_config),
    )

    # Note: we do not call save_train_results here because the base Featurizer used
    # for mask interventions does not implement to_dict() serialization. Scores and
    # feature_indices are saved manually below.

    single_result = result["results_by_key"][("single",)]
    train_score = single_result["train_score"]
    test_score = single_result["test_score"]

    # Compute n_features_by_unit (number of selected dimensions per unit)
    feature_indices = single_result.get("feature_indices", {})
    n_features_by_unit = {
        unit_id: len(indices) if indices is not None else 0
        for unit_id, indices in feature_indices.items()
    }

    logger.info(
        "Boundless DAS single-cell complete. train=%.3f, test=%.3f",
        train_score,
        test_score,
    )

    return {
        "boundless_result": {
            "train_score": train_score,
            "test_score": test_score,
            "n_features_by_unit": n_features_by_unit,
        },
    }
